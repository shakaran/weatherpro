...
#-----------------------------------------------------------------------
# 3.20 2015-009-05 release 2.8 version
#-----------------------------------------------------------------------
# Settings:
#
$display_astronomy      = true;
$display_lunar_cycle    = true;
$display_seasons        = true;
$display_suncalc	= true;
$display_moon_phases	= true;
$display_day_night_map	= true;
#
#-----------------------------------------------------------------------
...
#-----------------------------------------------------------------------
#
$this_script    = $pageFile;
$include_suncalc= 'inc/suncalc_frame.php';	// script to add suncalc to this page.
#-----------------------------------------------------------------------
#  define sun moon functions only when not used elsewhere
#-----------------------------------------------------------------------
ws_message ('<!-- module '.$this_script.' ('.__LINE__.'): check if moon functions exist -->');
if (!function_exists ('getUSNOsunmoon')) {
        ws_message ('<!-- module '.$this_script.' ('.__LINE__.'): moon functions do not exist -->');
# ----------------------------------------------------------------------
# getUSNOsunmoon
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
        function getSeasonInfo ($YY=0) { // feed it the year
               global $sunMoonInfo;
	        $seasonList = array(
                        #          march   |june    |septem  |december|
                        #year      day time|day time|day time|day time|
                        '2015' => '20T22:45|21T16:38|23T08:21|22T04:48|',
                        '2016' => '20T04:30|20T22:34|22T14:21|21T04:48|',
                        '2017' => '20T10:29|21T04:24|22T20:02|21T16:28|',
                        '2018' => '20T16:15|21T10:07|23T01:54|21T22:23|',
                        '2019' => '20T21:58|21T15:54|23T07:50|22T04:19|',
                        '2020' => '20T03:50|20T21:44|22T13:31|21T10:02|',
                        '2021' => '20T09:37|21T21:03|22T13:31|21T15:59|',
                        '2022' => '20T15:33|21T21:09|23T01:04|21T21:48|',
                        '2023' => '20T21:24|21T14:58|23T06:50|22T03:27|',
                        '2024' => '20T03:06|20T20:51|22T12:44|21T09:21|',
                        '2025' => '20T09:01|21T02:42|22T18:19|21T15:03 |',
	        );


                if($YY<2015) {$YY = date('Y');} // use current year 
                if(!isset($seasonList[$YY])) {
                           $sunMoonInfo['error'] .= "Year $YY not in list";
                           return $sunMoonInfo;
                }
                list($spring,$summer,$fall,$winter) = explode('|',$seasonList[$YY]);
                $sunMoonInfo['spring'] = strtotime ($YY.'03'.$spring.':00 UTC');
                $sunMoonInfo['summer'] = strtotime ($YY.'06'.$summer.':00 UTC');
                $sunMoonInfo['fall']   = strtotime ($YY.'09'.$fall.  ':00 UTC');
                $sunMoonInfo['winter'] = strtotime ($YY.'12'.$winter.':00 UTC');
                return true;
...
# ----------------------------------------------------------------------
        function usno_makeRequest($url,$PostParms='') {
                ...
                curl_close ($ch);
                ws_message ('<!-- module '.$this_script.' ('.__LINE__.') Data will be loaded using :'.$url.'.-->');
                ws_message ('<!-- module '.$this_script.' ('.__LINE__.') Data loaded = '.$rawData. '-->');
                if (trim($errors) <> ''){
                        ws_message ('<!-- module '.$this_script.' ('.__LINE__.') last error  = '.$errors. '-->',true);
                }
                return($rawData);
...
if (isset ($_REQUEST['force']) && $_REQUEST['force'] == 'usno') {
        $use_cache = false;
        ws_message ('<!-- module '.$this_script.' ('.__LINE__.'):  cacheFile not used, force was set to usno',true);
} 
#
if ($use_cache && file_exists($cacheName) &&  ( date ('j',filemtime($cacheName) ) == $today) ){
	ws_message ('<!-- module '.$this_script.' ('.__LINE__.'):  cacheFile  used '.date ($SITE['timeFormat'],filemtime($cacheName) ).' -->');
	$sunMoonInfo =  unserialize(file_get_contents($cacheName));
} 
else {  ws_message ('<!-- module '.$this_script.' ('.__LINE__.'):  loading '.$cacheName.' from USNO -->');
	$sunMoonInfo            = array();
	...
	if (!file_put_contents($cacheName, serialize($sunMoonInfo))){   
		 ws_message ('<!-- module '.$this_script.' ('.__LINE__.'):  Could not save data to cache '.$cacheName.'. Please make sure your cache directory exists and is writable. Program halts -->');
		 echo '<h3>Program ends as cache is not writable</h3>';
		 exit;
	} 
	else {  ws_message ('<!-- module '.$this_script.' ('.__LINE__.'):  data ('.$cacheName.') saved to cache  -->');
	}
}
ws_message ('<!-- module '.$this_script.' ('.__LINE__.'): Data to be processed:  <pre>'.print_r ($sunMoonInfo,true).'</pre> -->');
# ----------------------------------------------------------------------
...
if (isset ($skipMoonPage) && ($skipMoonPage == true) ){
	$ws['moonrise']		= date($timeFormat,$sunMoonInfo['moonrise']);
	$ws['moonset']		= date($timeFormat,$sunMoonInfo['moonset']);
...
<tr>              
<td><a href="#">
<img src="http://sohowww.nascom.nasa.gov/data/realtime/eit_304/512/latest.jpg" height="100" width="100" alt=" "  style = "border: 0;"
...
ws_message ('<!-- module '.$this_script.' ('.__LINE__.'): $SITE["latitude"] = '.$SITE['latitude'].'| $SITE["longitude"] = '.$SITE['longitude'].' ==> '.$strLatLon.' -->');
?>
<td rowspan="2">
...
-->
<img src="ajaxImages/moon<?php echo  $sunMoonInfo['pic']; ?>.gif" width="100" height="100" alt=""/>
...
<td>
<?php 
	echo 	langtransstr('sunrise').': '.	date($timeOnlyFormat,$sunriseDate).'<br />'.
		langtransstr('Sunset').': '. 	date($timeOnlyFormat,$sunsetDate).'<br />'.
		langtransstr('Daylight').': '.	$hoursofpossibledaylight;
?>
</td>
<td>
<?php 	$rise 	= langtransstr('Moonrise').': '.date($timeOnlyFormat,$moonriseDate);
	$set	= langtransstr('Moonset').': '. date($timeOnlyFormat,$moonsetDate);
	$now	= date($dateOnlyFormat,time());
	
	$extra 	= date($dateOnlyFormat,$moonriseDate);
	if ($now <> $extra)  {
		$rise  .= '<small> (' . $extra . ')</small>'; 
	}	  	
	$extra = date($dateOnlyFormat,$moonsetDate);
	if ($now <> $extra)  {
...
<?php 
# ----------------------------------------------------------------------
} // eof display astrnomy
if ($display_lunar_cycle) { 
# ----------------------------------------------------------------------
?>
<div class="blockDiv" style="background: black; color: white;">
...
<tr>
<td><?php echo date($timeFormat,$lunarFQ); 	?><br/><small>(utc) <?php echo gmdate($timeFormat,$lunarFQ); 	?></small></td>
<td><?php echo date($timeFormat,$lunarFM); 	?><br/><small>(utc) <?php echo gmdate($timeFormat,$lunarFM); 	?></small></td>
<td><?php echo date($timeFormat,$lunarLQ); 	?><br/><small>(utc) <?php echo gmdate($timeFormat,$lunarLQ); 	?></small></td>
<td><?php echo date($timeFormat,$lunarNextNM); 	?><br/><small>(utc) <?php echo gmdate($timeFormat,$lunarNextNM);?></small></td>
</tr>
</table>
</div>
<br />
...
echo 
'<td>'.	date($timeFormat,$marchequinox).'<br/><small>(utc)'.gmdate($timeFormat,$marchequinox).	'</small></td>
<td>'.	date($timeFormat,$junesolstice).'<br/><small>(utc)'.gmdate($timeFormat,$junesolstice).	'</small></td>
<td>'.	date($timeFormat,$sepequinox).  '<br/><small>(utc)'.gmdate($timeFormat,$sepequinox).	'</small></td>
<td>'.	date($timeFormat,$decsolstice). '<br/><small>(utc)'.gmdate($timeFormat,$decsolstice).	'</small></td>'.PHP_EOL;
} 
else { // Use Southern Hemisphere dates with images
'<td>'.	date($timeFormat,$sepequinox).  '<br/><small>(utc)'.gmdate($timeFormat,$sepequinox).  	'</small></td>
<td>'.	date($timeFormat,$decsolstice). '<br/><small>(utc)'.gmdate($timeFormat,$decsolstice). 	'</small></td>
<td>'.	date($timeFormat,$marchequinox).'<br/><small>(utc)'.gmdate($timeFormat,$marchequinox).	'</small></td>
<td>'.	date($timeFormat,$junesolstice).'<br/><small>(utc)'.gmdate($timeFormat,$junesolstice).	'</small></td>'.PHP_EOL;
}
?>
</tr>
</table>
...
# ----------------------------------------------------------------------
if ($display_suncalc) { 
        if (isset ($include_suncalc) && $include_suncalc == true) {
                include $include_suncalc;
                echo '<br />'.PHP_EOL;
        }
} // eof display_suncalc
# ----------------------------------------------------------------------
if ($display_moon_phases) { 
# ----------------------------------------------------------------------
?>
<div  class="blockDiv" style="background: black; color: white;">
...
<?php 
# ----------------------------------------------------------------------
} // eof display_moon_phases
# ----------------------------------------------------------------------
if ($display_day_night_map) { 
        include 'wsDashEarth.php';
}  // eo display_day_night_map
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# ----------------------  version history
# 3.20 2015-09-03 release 2.8 version 
