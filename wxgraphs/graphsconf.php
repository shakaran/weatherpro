...
#
#  #####    change these lines for template users !
#
# if you installed in a dfifferent folder change weather to your folder
$uploadDir      = $installed = '';
$uoms   = array();

function fetch_root() {
        global $uploadDir,$uoms;
        ob_start();
        $SITE['iconsDir'] = '';  // define $SITE array and missing setting
        include '../wsLoadSettings.php';
        $uploadDir      = $SITE['clientrawDir'];
        $uoms['temp']   = $SITE['uomTemp'];
        $uoms['rain']   = trim($SITE['uomRain']);
        $uoms['baro']   = trim($SITE['uomBaro']);
        $uoms['wind']   = trim($SITE['uomWind']);
        ob_clean();
        return;
}
fetch_root();
#
#  #####    normally you do not have to change anything below this point
#
$hostloc = '../'.$uploadDir;     //must have trailing slash
$jploc = "../jpgraph/src/";   			//must have trailing slash
#echo $hostloc.'--'.$jploc; exit;
...
$speed_unit = $uoms['wind'];    //Set this to the correct speed units
if ($speed_unit == 'm/s') {
        $speed_conv = 0.5150; //Knots to mph 1.1520, Knots to kmh 1.8540, Metres/Sec 0.5150, to leave as Knots use 1
}
elseif ($speed_unit == 'km/h') {
        $speed_conv = 0.5150; //Knots to mph 1.1520, Knots to kmh 1.8540, Metres/Sec 0.5150, to leave as Knots use 1
}
elseif ($uoms['wind'] == 'mph') {
        $speed_conv = 1.1520; //Knots to mph 1.1520, Knots to kmh 1.8540, Metres/Sec 0.5150, to leave as Knots use 1
}
else {  $speed_conv = 1; //Knots to mph 1.1520, Knots to kmh 1.8540, Metres/Sec 0.5150, to leave as Knots use 1
        $speed_unit = "kts";   //Set this to the correct speed units
}
...
if ($uoms['temp'] == '&deg;C') {
        $temp_conv = 1; // set to 1.8 if using Fahrenheit
        $temp_unit = "°C"; // Change to °F if you use Fahrenheit
}
else {  $temp_conv = 1.8; // set to 1.8 if using Fahrenheit
...
//Rain units
$rain_unit = $uoms['rain']; // Change to in when required
if ($rain_unit == 'mm') {
        $rain_conv = 1; // Change to 0.0394 when using inches use 1 for mm
        $rain_format = "%0.1f"; //Sets no of decimal places to display use %0.2f when using inches and %0.1f when using mm
}
else {  $rain_conv = 0.0394; // Change to 0.0394 when using inches use 1 for mm        
...
//Pressure units
$pres_unit = $uoms['baro']; //Set to required value
if ($pres_unit == 'hPa' || $pres_unit == 'mb' ) {
        $pres_conv = 1; // Set to 0.0295 for in of Mercury or 1 for hPa/mb
        $pres_format = "%0.0f"; //Sets no of decimal places to display use %0.2f when using inches and %0.0f when using hPa/MBmm
        $pres_automin = 940; // Sets the min Y axis value, suggest 27 for in and 950 for hPa/MB
        $pres_automax = 1050; // Sets the max Y axis value, suggest 31 for in and 1050 for hPa/MB
} else {$pres_conv = 0.0295; // Set to 0.0295 for in of Mercury or 1 for hPa/mb
...
//Solar units
$solar_unit = "w/m2"; // change to % if you use the percentage value for graphing
