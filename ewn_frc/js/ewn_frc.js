...
$(function () {
  ...
  $(".ewnnav").click(function () {
    ...
    if(l>0){
      ...
      if(target=="stats_table"){
        rgr=-1;
        chart = new Highcharts.Chart(voptions);
      }
...
if(!map_only){
 ...
 Highcharts.setOptions({
  ...
  var xAxis4 = { // Bottom X axis
      type: 'datetime',
      tickInterval: 7 * 24 * 36e5, // two hours
      tickLength: 0,
      gridLineWidth: 0,
      gridLineColor: (Highcharts.theme && Highcharts.theme.background2) || '#F0F0F0',
      startOnTick: false,
  ...
  var tempYAxisb = [
    {
      title: {text: null},labels: {format: '{value}%',style: yLabels,x: -3},
      maxPadding: 0.3,tickInterval: 5,gridLineWidth: 0,max:100
    }];
  var voptions = {
    chart: {marginBottom: 77,renderTo: "vorgraph"},
    tooltip:{
      formatter: function () { 
        var index = this.points[0].point.index;
        var ret = "<b>" + Highcharts.dateFormat('%Y-%m-%d', this.x) +"</b>";
        ret += '<table>';
        Highcharts.each(this.points, function (point) {
          var series = point.series;
          ret += '<tr><td><span style="color:' + series.color + '">\u25CF</span> ' + series.name +
    ...
    yAxis: tempYAxisb,
    series: [{
      name: "EWN",data: smoothline(vewn),tooltip: {valueSuffix: "%"},
      zIndex: 1,color: '#008B00',pointInterval: 86400000,pointStart: vstart
    },{
      name: "WRFDA",data: smoothline(vwrfda),tooltip: {valueSuffix: "%"},
      zIndex: 1,color: '#FF3333',pointInterval: 86400000,pointStart: vstart
    },{
      name: "DWD",data: smoothline(vdwd),tooltip: {valueSuffix: "%"},
      zIndex: 1,color: '#EE7600',pointInterval: 86400000,pointStart: vstart
    },{
      name: "YR",data: smoothline(vyr),tooltip: {valueSuffix: "%"},
      zIndex: 1,color: '#20B2AA',pointInterval: 86400000,pointStart: vstart
    },{
      name: "SMHI",data: smoothline(vsmhi),tooltip: {valueSuffix: "%"},
      zIndex: 1,color: '#CD5B45',pointInterval: 86400000,pointStart: vstart
    },{
      name: "FMI",data: smoothline(vfmi),tooltip: {valueSuffix: "%"},
      zIndex: 1,color: '#9A32CD',pointInterval: 86400000,pointStart: vstart
    },{
      name: "GFS",data: smoothline(vgfs),tooltip: {valueSuffix: "%"},
      zIndex: 1,color: '#8B8B7A',pointInterval: 86400000,pointStart: vstart
    }]
  };
