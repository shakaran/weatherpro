...
switch ($datetime_col) {
    case 'yes':
        $que = "SELECT datetime, $datacol FROM $dbtable WHERE datetime BETWEEN '$start' AND '$end' ORDER BY datetime ASC";
        break;
    case 'no':
        $que = "SELECT datetime, $datacol FROM(SELECT CONCAT(date,' ',time)as datetime, $datacol from $dbtable) as temp WHERE datetime BETWEEN '$start' AND '$end' ORDER BY datetime ASC";            
}
