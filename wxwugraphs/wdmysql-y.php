...
// CACHING
$dbcFile = $WUcacheDir.'dbc-year-'.$year.'.txt';
$db_caching = $db_cache_type == 'disabled' || $db_cache_type == false ? false : true; 
if ($db_caching) {
  ...
  if ($db_cache_type == 'file') {
    ...
    $csq = mysql_query("SELECT data,last_access FROM $db_cache_table WHERE id='year-$year'");
  ...
  $ReqYr = $year;
  $TYr = date('Y');
  $endReqYr = strtotime($year.'-12-31 23:59:59'); 
  if ($fromCache) {
    if ($ReqYrt != $TYr && $last_access <= $endReqYr) { // recreate uncomplete year
      $fromCache = false;
      $updCache = true;    
    } elseif ($ReqYr == $TYr and ($last_access + ($mysqlYR*60*60)) < time()) { // recreate today cache if expired refresh interval
      $fromCache = false;
      $updCache = true;    
    } else {
      $updCache = false;
...
//if (!$fromCache || empty($outData)) {
  ...
  while ($row = mysql_fetch_array($sql, MYSQL_ASSOC)) {
    ...
    if ($chDay != $actDay || $qend) { // day is changed, calculate/add previous day
      ...
      //echo $actrM.' != '.$chMo.' && '.$ysign.' == '.$actrY.'<br>';
      if ($actrM != $chMo && $ysign == $actrY) { // month changed, create previous month data;
      ...
      $monthrSum += round($dailyRain*$r_rain, 2);
      $premo = $ysign == $actrY ? $actrM*1 : 12; // We have in this year also data from 1.1. of next year so we use $ysign
