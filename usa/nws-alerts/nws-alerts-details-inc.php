...
// Version: 1.00  12-Mar-2006 - Initial release

$Version = "nws-alerts-details-inc.php - 1.00 - 28-July-2012 - Initial release"; 
...
<div style="width: 630px; margin: 0px auto 6px auto; border: 1px solid #000; background-color:#EFEFEF">
 <table style="width: 600px; margin: 10px auto 14px auto; border: none;">
  <tr>
...
if (!empty($czCode)) {                                                                          // IF there are alerts for the location
    ...
    (!empty($aav[2])) ? $intensity = $aav[2] : $intensity = ' - - -';                           //     get intensity
      ...
      $sfll[] = explode(' ', $sf);                                                              //       create array of coordinates for shape files
		...
		if(!empty($intensity) and $intensity == 'Moderate') {
			$intensity = '<span style="color: #339"><b> &nbsp;'.$intensity.'&nbsp;</b> </span>';}    //     change intensity color
  ...
  <a  id="WA'.$aav[13].'"></a>
  <table  style="width: 610px; margin:10px auto 14px auto; border: none;">
   <tr>
...
if(file_exists('usa/nws-alerts/nws-shapefile.txt') && !empty($zCode) && !empty($haveJS)) {           // IF the shape file exists and a valid code
  // get data from shapefile for google map
  $nsf = trim(file_get_contents('usa/nws-alerts/nws-shapefile.txt'));                                //   get nws shape file
...
<div style="width:632px; margin:0px auto 0px auto;">
 <table style="width:100%; margin:0px auto 0px auto; border:1px solid black; background-color:#F5F8FE">
  <tr>
