...
foreach($daily_log as $log =>$data) {  // FOR EACH alert in the log file
  ...
  (!empty($data[2])) ? $issued = date("D g:i a",$data[2]) : $issued = '';   //   set issue date
  (!empty($data[3])) ? $expired = date("D g:i a",$data[3]) : $expired = ''; //   set expire date
...
//    print_r($rcount);  echo "<br />\n";
    ...
    if(isset($rdv)) {                     // IF there is an alert update
      foreach($rdv as $rdk2 => $rdv2) {   //   FOR EACH alert update
        ...
        $tdd .= '<tr>'."\n";
        $tdd .= '<td class="datahead" colspan="3" style="background-image: url('.$icons_folder.'/lbg.png); color:#000"><b>'.$rdv2[0][4].strtoupper($rdk2).'</b> &nbsp; '.$revsd.'</td>'."\n";
        $tdd .= '</tr>'."\n";
        $rdv22c = 0;
        ...
        if(isset($rdv2[0])) {                              // IF there is an alert update
          foreach($rdv2 as $rdk22 => $rdv22) {             //   FOR EACH update
            ...
            ($rdv22c&1) ? $tbc = '#EEE' : $tbc = '#FFF';   //     set table background color
            ...
            $tdd .= '<td style="width:33%; text-align:center; font-size:90%; background-color:'.$tbc.'; border-right:dotted thin #777; padding:0px 0px 4px"><b>Expires:</b> '.$rdv22[3].'</td>'."\n";
