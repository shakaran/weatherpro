...
*/
$Version = "nws-alerts.php - 1.23 - 23-Jan-2014 - NWS Public Alerts"; 
...
// set variable in case it isn't in the config file
$logAlerts = false;
...
if($cmyzc > 4 and $noCron == true) {
  echo 'nws-alerts: Please consult the <a href="./usa/nws-alerts/NWS-alerts_read_me.html" target="_blank"><b>readme here</b></a> as checking more than four warning/county codes can delay the loading of your pages. 
You should use a cron job to get the data.<br /><br />'; 
...
if(!isset($noAlertText)) {
  ...
  $noted .= "<!-- nws-alerts-config.php has not been updated -->\n";
}
...
if($updateCache) {
  ...
  foreach ($codeID as $ck => $cv) {                                                                           // FOR EACH primary alert
    if(isset($cv)) {                                                                                          //   IF there is an alert
      ...
      // check for flood type
			if($floodType) {                                       // IF Flood type option is on
        foreach($czR->info->parameter AS $element) {         //  FOR EACH parameter
          if($element->valueName == 'VTEC' ) {               //   IF there is VTEC
            ...
            if(preg_match("/\.FA\.\w\.\w/",$vtec2)) {        //    IF FA is found in the value
      ...
      if(!preg_match("/expired/",$note)) {                                                                    //     IF alert hasn't expired
  ...
  if($useAlertBox and empty($atData)) {                                                      // IF using alert box and no alerts
    ...
    $box .= "\n<!-- nws-alerts box -->\n"
         ...
         .';  margin:0px auto 0px auto;">'."\n";  ### leuven template  removed border:solid thin #000;
...
';
  ...
  if($useAlertBox and !empty($atData)) {                                                       // IF use alert box & have data
    ...
    $aksba = array_keys($soryByALert);                                                          // get the most severity keys
    ...
    // set alert box style   ## removed border for leuven template
    $box .= "\n<!-- nws-alerts box -->\n"
         .'<div style="width:'.$aBox_Width
         .'; margin:0px auto 0px auto;">'."\n"
    ...
    if($sortbyEvent == 0) {
      foreach ($sortMe as $sblk => $sblv) {
        ...
        $box .= '  <span style="white-space: nowrap">&nbsp;<a href="'.$alertURL.'&amp;a='				#####  changed for leuven-template
             .$sblv[0][0][2].$skiptopText.'" '.$setStyle.' title=" &nbsp;View details"><b>'.$abt
        ...
        for($i=0;$i<$csblv;$i++) {                                                               //    FOR EACH string of data
          ...
          $box .= '<span style="white-space: nowrap">&nbsp;'.$sblv[$i][0][4]
               .'&nbsp;<a href="'.$alertURL.'&amp;a='.$sblv[$i][0][2].'#WA'.$sblv[$i][0][6]		// #####  changed leuven template
    ...
    if($sortbyEvent == 1) {
      ...
      foreach ($sortMe as $sblk => $sblv) {                                                      //    FOR EACH location with data
        $box .= '  <span style="white-space: nowrap">&nbsp;<a href="'.$alertURL
             .'&amp;a='.$sblv[key($sblv)][0][2].$skiptopText.'" '.$setStyle
...
function get_nwsalerts($url)    {
  ...
  $data = curl_exec($ch);
  if($data === false or preg_match('/Connection Issue|invalid zone/Ui',$data)) {  // could not connect
    $data = '';
