...
$pageVersion	= '3.20 2015-09-25  ALT pre 5.4';
#-------------------------------------------------------------------------------
# 3.20 2015-09-25 release 2.8 version  plus removed < 5.4 php facilities, Notice: Use of undefined constant JSON_UNESCAPED_SLASHES 
#-------------------------------------------------------------------------------
...
# file locationsd world
	...
	for ($n1 = 0; $n1 < $count; $n1++) {
		...
		if ($found == true) {
			...
			$arr_countries[$country]	= '1';
			if ($largest_lat == 0   || $largest_lat   < $arr['lat'])  {$largest_lat   = $arr['lat'];}
			if ($smallest_lat == 0  || $smallest_lat  > $arr['lat'])  {$smallest_lat  = $arr['lat'];}
			if ($largest_long == 0  || $largest_long  < $arr['long']) {$largest_long  = $arr['long'];}
			if ($smallest_long == 0 || $smallest_long > $arr['long']) {$smallest_long = $arr['long'];}
		}
	}
...
$json_out      .= json_encode ($arr_conditions);
$json_out      .= ','.PHP_EOL.'"nets":'.PHP_EOL;
$json_out      .= json_encode ($arr_raw['nets']);
