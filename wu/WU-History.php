...
$Langtitle  = langtransstr("Station Historical Data");            // The Bold text at the top
$LcurDay    = langtransstr("Return to Current Day");              // The link back to the current day if off in one of the other modes.  "" to disable it.
// Optional footer bar
...
$toTop      = true;                                // Most will want this, but some folks have alternative methods. 
$LtopPg     = langtransstr("Return to Top");
...
$skipSolar  = true;                                 // true to skip solar data, false to include them
...
$mnthname     = array(langtransstr('Nil'), langtransstr('January'), langtransstr('February'), langtransstr('March'), 
					langtransstr('April'), langtransstr('May'), langtransstr('June'), langtransstr('July'), langtransstr('August'), 
					langtransstr('September'), langtransstr('October'), langtransstr('November'), langtransstr('December'));
              // Next are for the Summary Table
$Langtabs     = array(langtransstr('Daily'), langtransstr('Weekly'), langtransstr('Monthly'), langtransstr('Yearly'), langtransstr('Custom'));  // Tabs above the Summary Table
$LangSumHeads = array(langtransstr("Temperature"), langtransstr("Dew Point"), langtransstr("Humidity"),
					langtransstr("Wind Speed"), langtransstr("Wind Gust"), langtransstr("Wind"), 
					langtransstr("Pressure"), langtransstr("Precipitation"), langtransstr("Solar"));
              // Headings for the Summary Table
$LangSumCols  = array(langtransstr("Current"), langtransstr("High"), langtransstr("Low"), langtransstr("Average"));
...
$Lprev     = langtransstr("Previous");
$Ltarget   = array(langtransstr("Day"), langtransstr("Week"), langtransstr("Month"), langtransstr("Year"));
$Lview     = langtransstr("View");
$Ltab1     = langtransstr("Tabular Data for");  // Next 5 for Blue bar above the Tabular listing
$Ltab2     = langtransstr("Weeks Tabular Data");
$Ltab3     = langtransstr("Months Tabular Data");
$Ltab4     = langtransstr("Years Tabular Data");
$Ltab5     = langtransstr("Custom Date Range Tabular Data");
          // Next are for the Tabular Table
$Lheadings = array(langtransstr('Time'), langtransstr('Temperature'), langtransstr('Dew Point'), langtransstr('Pressure'), langtransstr('Wind'), 
				langtransstr('Wind Speed'), langtransstr('Wind Gust'), langtransstr('Humidity'), langtransstr('Rainfall Rate (Hourly)'), 
				langtransstr('solar'), langtransstr('Conditions'));
          // Headings when in weekly, monthly etc modes
$Lhdngs2   = array(langtransstr("Temp"), langtransstr("Dew Point"), langtransstr("Humidity"), langtransstr("Sea Level Pressure"), langtransstr("Wind"),
				langtransstr("Gust Speed"), langtransstr("Precip"));
$Lcols2    = array(langtransstr("high"), langtransstr("ave"), langtransstr("low"), langtransstr("sum")); 
$Lcommafile= langtransstr("Comma Delimited File");
$Lthanks   = langtransstr("Compliments of");
//
...
if (isset($SITE['uomTemp']))	{ $units	= preg_match('|C|i',$SITE['uomTemp']) ? 'M':'E';}
if (isset($SITE['WUunits']))	{$units		= $SITE['WUunits'];}
if (isset($SITE['organ'])) 		{$LangFtext = $SITE['organ'];}
if (isset($SITE['wuStart'])) 	{$birthday	= $SITE['wuStart'];}
...
// Display link back to current day if not currently showing "today"
	if (($mode > 1 or ! $isToday) && $LcurDay <> "") {
		$callstr = $PHP_SELF . '&ID=' . $WUID . '&amp;month=' . date("m") . '&amp;day=' . date("d") . '&amp;year=' . date("Y") . '&amp;mode=1&amp;units=' . $units.$phpselftop;
...
?>
	<table cellspacing="0" cellpadding="0" style="width: <?php echo($pwidth); ?>;">
		...
		<td class="full">
			...
			if ($units == "M") {
			echo $Lunits . ': &nbsp;<b>' . $Lmetric . '</b>&nbsp;&nbsp;&nbsp; <a href="' . $callstr . '&amp;units=E'.$phpselftop . '">' . $Lenglish . '</a>&nbsp;&nbsp;&nbsp;<a href="' . $callstr . 'B'.$phpselftop.'">' . $Lboth . '</a>';
			} elseif ($units == "E") {
			echo $Lunits . ': &nbsp;<b>' . $Lenglish . '</b>&nbsp;&nbsp;&nbsp; <a href="' . $callstr . '&amp;units=M'.$phpselftop . '"> ' . $Lmetric . '</a>&nbsp;&nbsp;&nbsp;<a href="' . $callstr . 'B'.$phpselftop.'">' . $Lboth . '</a>';
			} else {
			echo $Lunits . ': &nbsp;<b>' . $Lboth . '</b>&nbsp;&nbsp;&nbsp; <a href="' . $callstr . '&amp;units=E'.$phpselftop . '">' . $Lenglish . '</a>&nbsp;&nbsp;&nbsp;<a href="' . $callstr . 'M'.$phpselftop.'">' . $Lmetric . '</a>';
			}
			?>
			</td>
			<td class="hRight"></td>
...
if ($mode == 1) {
	...
	echo '<td class="nobr noprint"><a href="' . $PHP_SELF . '&ID=' . $WUID . '&amp;month=' . $ndate[mon] . '&amp;day=' . $ndate[mday] . '&amp;year=' . $ndate[year] . '&amp;units=' . $units . '&amp;mode=' . $mode .$phpselftop. '">&laquo; ' . $Lprev . ' ' . $Ltarget[0]. '</a></td>';
} elseif ($mode == 2) {
	$ndate = AddDate($mo, $da, $yr, -7);
	echo '<td class="nobr noprint"><a href="' . $PHP_SELF . '&ID=' . $WUID . '&amp;month=' . $ndate[mon] . '&amp;day=' . $ndate[mday] . '&amp;year=' . $ndate[year] . '&amp;units=' . $units . '&amp;mode=' . $mode .$phpselftop  . '">&laquo; ' . $Lprev . ' ' . $Ltarget[1] . '</a></td>';
} elseif ($mode == 3) {
	$ndate = AddDate($mo, $da, $yr, -30);
	echo '<td class="nobr noprint"><a href="' . $PHP_SELF . '&ID=' . $WUID . '&amp;month=' . $ndate[mon] . '&amp;day=' . $ndate[mday] . '&amp;year=' . $ndate[year] . '&amp;units=' . $units . '&amp;mode=' . $mode .$phpselftop  . '">&laquo; ' . $Lprev . ' ' . $Ltarget[2]. '</a></td>';
} elseif ($mode == 4) {
	$ndate = AddDate($mo, $da, $yr, -365);
	echo '<td class="nobr noprint"><a href="' . $PHP_SELF . '&ID=' . $WUID . '&amp;month=' . $ndate[mon] . '&amp;day=' . $ndate[mday] . '&amp;year=' . $ndate[year] . '&amp;units=' . $units . '&amp;mode=' . $mode .$phpselftop  . '">&laquo; ' . $Lprev . ' ' . $Ltarget[3]. '</a></td>';
}
?>
...
if ($mode == 1) {
	$ndate = AddDate($mo, $da, $yr, +1);
	echo '<td class="nobr noprint"><a href="' . $PHP_SELF . '&ID=' . $WUID . '&amp;month=' . $ndate[mon] . '&amp;day=' . $ndate[mday] . '&amp;year=' . $ndate[year] . '&amp;units=' . $units . '&amp;mode=' . $mode.$phpselftop . '">' . $Lnext . ' ' . $Ltarget[0] . ' &raquo;</a></td>';
} elseif ($mode == 2) {
	$ndate = AddDate($mo, $da, $yr, +7);
	echo '<td class="nobr noprint"><a href="' . $PHP_SELF . '&ID=' . $WUID . '&amp;month=' . $ndate[mon] . '&amp;day=' . $ndate[mday] . '&amp;year=' . $ndate[year] . '&amp;units=' . $units . '&amp;mode=' . $mode.$phpselftop  . '">' . $Lnext . ' ' . $Ltarget[1] . ' &raquo;</a></td>';
} elseif ($mode == 3) {
	$ndate = AddDate($mo, $da, $yr, +30);
	echo '<td class="nobr noprint"><a href="' . $PHP_SELF . '&ID=' . $WUID . '&amp;month=' . $ndate[mon] . '&amp;day=' . $ndate[mday] . '&amp;year=' . $ndate[year] . '&amp;units=' . $units . '&amp;mode=' . $mode.$phpselftop  . '">' . $Lnext . ' ' . $Ltarget[2] . ' &raquo;</a></td>';
} elseif ($mode == 4) {
	$ndate = AddDate($mo, $da, $yr, +365);
	echo '<td class="nobr noprint"><a href="' . $PHP_SELF . '&ID=' . $WUID . '&amp;month=' . $ndate[mon] . '&amp;day=' . $ndate[mday] . '&amp;year=' . $ndate[year] . '&amp;units=' . $units . '&amp;mode=' . $mode .$phpselftop . '">' . $Lnext . ' ' . $Ltarget[3] . ' &raquo;</a></td>';
}
?>
		</tr>
	</table>
...
if ($mode == 1) { echo '<td ' . $ActTab . '>' . $Langtabs[0] . '</td>' . "\n"; } else { echo '<td ' . $InacTab . '><a href="' . $callstr . '&amp;mode=1'.$phpselftop.'">' . $Langtabs[0] . '</a></td>' . "\n"; }
if ($mode == 2) { echo '<td ' . $ActTab . '>' . $Langtabs[1] . '</td>' . "\n"; } else { echo '<td ' . $InacTab . '><a href="' . $callstr . '&amp;mode=2'.$phpselftop.'">' . $Langtabs[1] . '</a></td>' . "\n"; }
if ($mode == 3) { echo '<td ' . $ActTab . '>' . $Langtabs[2] . '</td>' . "\n"; } else { echo '<td ' . $InacTab . '><a href="' . $callstr . '&amp;mode=3'.$phpselftop.'">' . $Langtabs[2] . '</a></td>' . "\n"; }
if ($mode == 4) { echo '<td ' . $ActTab . '>' . $Langtabs[3] . '</td>' . "\n"; } else { echo '<td ' . $InacTab . '><a href="' . $callstr . '&amp;mode=4'.$phpselftop.'">' . $Langtabs[3] . '</a></td>' . "\n"; }
if ($mode == 5) { echo '<td ' . $ActTab . '>' . $Langtabs[4] . '</td>' . "\n"; } else { echo '<td ' . $InacTab . '><a href="' . $callstr . '&amp;mode=5'.$phpselftop.'">' . $Langtabs[4] . '</a></td>' . "\n"; }
?>
		</tr>
	</table>
...
?>
	<div class="taC tm10">
	<img src="<?php echo($WUgraphstr); ?>?day=<?php echo($da); ?>&amp;year=<?php echo($yr); ?>&amp;month=<?php echo($mo); ?>&amp;ID=<?php echo($WUID); ?>&amp;width=<?php echo($gwidth); ?>&amp;showsolarradiation=<?php if ($mode < 3) echo $gsolar?'1':'0';?>&amp;showuv=<?php if ($mode < 3) echo $guv?'1':'0'; ?>&amp;showtemp=<?php echo $gtemp?'1':'0'; ?>&amp;showpressure=<?php echo $gpress?'1':'0'; ?>&amp;showwind=<?php echo $gwind?'1':'0'; ?>&amp;showwinddir=<?php echo $gwindir?'1':'0'; ?>&amp;showrain=<?php echo $grain?'1':'0'; ?>&amp;type=<?php echo($gphrase).$phpselftop; ?>" alt="Historical Graphs" id="wxHistoryImage" />
	<div class="noGap">&nbsp;</div>
	</div>
		...
		for ($row=0; $row<count($csvarray); $row++) {
			...
			for ($col=0; $col<$columns; $col++) {
				...
				if ($col == 0) {
					...
					$data = '<td class = "date">' . substr($tdate[weekday], 0, 3) . '</td><td class = "date"><a href="' . $PHP_SELF . '&ID=' . $WUID . '&amp;month=' . $pmo . '&amp;day=' .$pda . '&amp;year=' . $pyr . '&amp;mode=1&amp;units=' . $units.$phpselftop . '">' . $pda . '</a></td>';  //Date/Time
				}
				if ($col == 1) {
					$convarray = convertTemps($data);
...
?>
	<table class="center tm20 noprint">
		<tr>
			<td class = "logo"></td>
			<td><?php echo $Lthanks . ' '; ?><a href="http://www.wunderground.com"> Weather Underground</a> - <?php echo convert(memory_get_usage(true)); ?></td>
		</tr>
	</table>
  </td>
</tr>
